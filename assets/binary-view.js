...
class BinaryViewer {
  constructor(host, record, previousState, onCopy) {
    ...
    this.root.innerHTML = `
      ...
      <details class="hex-help"><summary>How to read and use this view</summary><p id="hex-keyboard-help">Offsets on the left are hexadecimal. The middle shows each original byte; the right shows printable ASCII (20–7E), with a dot for other bytes. Click either representation to inspect the byte. Drag, Shift-click, or hold Shift with the arrow keys to select a range. Arrows move by byte/row; Home/End move within a row; Ctrl/⌘+Home/End move to the first/last byte. Page Up/Down move a screen. Ctrl/⌘+A selects the whole body, Ctrl/⌘+C copies the selection, and Tab leaves the grid.</p><p>Find accepts pairs of hex digits, spaced or joined, or case-sensitive UTF-8 text. Previous/Next wrap through the body; F3 / Shift+F3 repeat the search. Ctrl/⌘+F and Ctrl/⌘+G open find and offset controls while the grid has focus. Hex/ASCII selections always represent the same bytes. Numeric values use the selected byte order, including exact 64-bit integers. UTF-8 text copying replaces invalid sequences with �; use Hex, Base64, or Save selection to preserve arbitrary bytes.</p><p>This is a read-only view of the captured body, available for text and binary payloads. Only visible rows are rendered; the entire captured body remains navigable. On touch screens, tap to select and swipe to scroll. Expand gives the viewer more room. Raw body downloads remain available above.</p></details>`;
